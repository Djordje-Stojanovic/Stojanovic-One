<script lang="ts">
	...
	$: sortedAndFilteredItems = regularReviewItems
		.filter(
			(item) =>
	...
	onMount(async () => {
		if ($session) {
			await fetchRegularReviewItems();
		}
	});

	async function fetchRegularReviewItems() {
		const { data, error } = await supabase
			.from('regular_review_items')
			.select('*')
			.eq('user_id', $session.user.id);

		if (error) {
			console.error('Error fetching regular review items:', error);
		} else {
			regularReviewItems = data;
		}
		loading = false;
	}
	...
	async function deleteStock(item) {
		if (confirm(`Are you sure you want to delete ${item.symbol} from your regular review list?`)) {
			const { error } = await supabase.from('regular_review_items').delete().eq('id', item.id);

			if (error) {
				console.error('Error deleting stock:', error);
			} else {
				regularReviewItems = regularReviewItems.filter((i) => i.id !== item.id);
			}
		}
	}
</script>

<h2 class="mb-4 text-2xl font-semibold text-gray-800 dark:text-gray-200">Regular Review</h2>
...
{#if loading}
	<p>Loading regular review items...</p>
{:else}
