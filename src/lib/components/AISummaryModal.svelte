<script lang="ts">
    ...
    $: modelName = selectedModel.split('/')[1].replace(':free', '');
                    ...
                    <div class="prose dark:prose-invert max-w-none">
                        {#each summary.split('\n') as line}
                            {#if line.startsWith('**') && line.endsWith('**')}
                                <h3 class="text-2xl font-bold bg-gradient-to-r from-blue-600 to-blue-400 bg-clip-text text-transparent mt-12 mb-6 pb-3 border-b border-gray-200 dark:border-gray-700">
                                    {line.replace(/\*\*/g, '')}
                                </h3>
                            ...
                            {:else if line.trim().startsWith('+')}
                                <p class="mb-2 ml-6 text-gray-700 dark:text-gray-300 flex items-start">
                                    <span class="mr-3 text-blue-500">•</span>
                                    <span class="flex-1">{line.replace('+', '').trim()}</span>
                                </p>
                            {:else if line.trim()}
                                <p class="mb-4 text-gray-700 dark:text-gray-300 leading-relaxed tracking-normal">
                                    {line}
                                </p>
                            {/if}
                        {/each}
