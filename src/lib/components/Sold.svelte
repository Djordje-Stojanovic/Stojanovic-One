<script lang="ts">
	...
	$: sortedAndFilteredItems = soldItems
		.filter(
			(item) =>
	...
	async function deleteStock(item) {
		if (confirm(`Are you sure you want to delete ${item.symbol} from your sold list?`)) {
			const { error } = await supabase.from('sold_items').delete().eq('id', item.id);

			if (error) {
				console.error('Error deleting stock:', error);
			} else {
				soldItems = soldItems.filter((i) => i.id !== item.id);
			}
		}
	}
</script>

<h2 class="mb-4 text-2xl font-semibold text-gray-800 dark:text-gray-200">Sold</h2>
