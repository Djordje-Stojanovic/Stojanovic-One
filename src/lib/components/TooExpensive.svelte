<script lang="ts">
	...
	$: sortedAndFilteredItems = tooExpensiveItems
		.filter(
			(item) =>
	...
	onMount(async () => {
		if ($session) {
			await fetchTooExpensiveItems();
		}
	});

	async function fetchTooExpensiveItems() {
		const { data, error } = await supabase
			.from('too_expensive_items')
			.select('*')
			.eq('user_id', $session.user.id);

		if (error) {
			console.error('Error fetching too expensive items:', error);
		} else {
			tooExpensiveItems = data;
		}
		loading = false;
	}
	...
	async function deleteStock(item) {
		if (confirm(`Are you sure you want to delete ${item.symbol} from your too expensive list?`)) {
			const { error } = await supabase.from('too_expensive_items').delete().eq('id', item.id);

			if (error) {
				console.error('Error deleting stock:', error);
			} else {
				tooExpensiveItems = tooExpensiveItems.filter((i) => i.id !== item.id);
			}
		}
	}
</script>

<h2 class="mb-4 text-2xl font-semibold text-gray-800 dark:text-gray-200">Too Expensive</h2>
...
{#if loading}
	<p>Loading too expensive items...</p>
{:else}
