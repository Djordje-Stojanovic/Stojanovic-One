<script lang="ts">
	...
	$: sortedAndFilteredItems = permanentPassItems
		.filter(
			(item) =>
	...
	onMount(async () => {
		if ($session) {
			await fetchPermanentPassItems();
		}
	});

	async function fetchPermanentPassItems() {
		const { data, error } = await supabase
			.from('permanent_pass_items')
			.select('*')
			.eq('user_id', $session.user.id);

		if (error) {
			console.error('Error fetching permanent pass items:', error);
		} else {
			permanentPassItems = data;
		}
		loading = false;
	}
	...
	async function deleteStock(item) {
		if (confirm(`Are you sure you want to delete ${item.symbol} from your permanent pass list?`)) {
			const { error } = await supabase.from('permanent_pass_items').delete().eq('id', item.id);

			if (error) {
				console.error('Error deleting stock:', error);
			} else {
				permanentPassItems = permanentPassItems.filter((i) => i.id !== item.id);
			}
		}
	}
</script>

<h2 class="mb-4 text-2xl font-semibold text-gray-800 dark:text-gray-200">Permanent Pass</h2>
...
{#if loading}
	<p>Loading permanent pass items...</p>
{:else}
