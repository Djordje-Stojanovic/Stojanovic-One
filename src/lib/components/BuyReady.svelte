<script lang="ts">
	...
	$: sortedAndFilteredItems = buyReadyItems
		.filter(
			(item) =>
	...
	onMount(async () => {
		if ($session) {
			await fetchBuyReadyItems();
		}
	});

	async function fetchBuyReadyItems() {
		const { data, error } = await supabase
			.from('buy_ready_items')
			.select('*')
			.eq('user_id', $session.user.id);

		if (error) {
			console.error('Error fetching buy ready items:', error);
		} else {
			buyReadyItems = data;
		}
		loading = false;
	...
	async function deleteStock(item) {
		if (confirm(`Are you sure you want to delete ${item.symbol} from your buy ready list?`)) {
			const { error } = await supabase.from('buy_ready_items').delete().eq('id', item.id);

			if (error) {
				console.error('Error deleting stock:', error);
			} else {
				buyReadyItems = buyReadyItems.filter((i) => i.id !== item.id);
			}
		}
	}
</script>

<h2 class="mb-4 text-2xl font-semibold text-gray-800 dark:text-gray-200">Buy Ready</h2>
		...
		{#each sortedAndFilteredItems as item (item.id)}
			<div class="rounded-lg border bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
				...
				<div class="mt-4 flex justify-between">
					<button
						on:click={() => moveToList(item, 'core-holdings')}
