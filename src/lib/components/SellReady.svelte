<script lang="ts">
	...
	$: sortedAndFilteredItems = sellReadyItems
		.filter(
			(item) =>
	...
	onMount(async () => {
		if ($session) {
			await fetchSellReadyItems();
		}
	});

	async function fetchSellReadyItems() {
		const { data, error } = await supabase
			.from('sell_ready_items')
			.select('*')
			.eq('user_id', $session.user.id);

		if (error) {
			console.error('Error fetching sell ready items:', error);
		} else {
			sellReadyItems = data;
		}
		loading = false;
	...
	async function deleteStock(item) {
		if (confirm(`Are you sure you want to delete ${item.symbol} from your sell ready list?`)) {
			const { error } = await supabase.from('sell_ready_items').delete().eq('id', item.id);

			if (error) {
				console.error('Error deleting stock:', error);
			} else {
				sellReadyItems = sellReadyItems.filter((i) => i.id !== item.id);
			}
		}
	}
</script>

<h2 class="mb-4 text-2xl font-semibold text-gray-800 dark:text-gray-200">Sell Ready</h2>
