<script lang="ts">
	...
	$: sortedAndFilteredItems = coreHoldingsItems
		.filter(
			(item) =>
	...
	onMount(async () => {
		if ($session) {
			await fetchCoreHoldingsItems();
		}
	});

	async function fetchCoreHoldingsItems() {
		const { data, error } = await supabase
			.from('core_holdings_items')
			.select('*')
			.eq('user_id', $session.user.id);

		if (error) {
			console.error('Error fetching core holdings items:', error);
		} else {
			coreHoldingsItems = data;
		}
		loading = false;
	}
	...
	async function deleteStock(item) {
		if (confirm(`Are you sure you want to delete ${item.symbol} from your core holdings list?`)) {
			const { error } = await supabase.from('core_holdings_items').delete().eq('id', item.id);

			if (error) {
				console.error('Error deleting stock:', error);
			} else {
				coreHoldingsItems = coreHoldingsItems.filter((i) => i.id !== item.id);
			}
		}
	}
</script>

<h2 class="mb-4 text-2xl font-semibold text-gray-800 dark:text-gray-200">Core Holdings</h2>
...
{#if loading}
	<p>Loading core holdings items...</p>
{:else}
		...
		{#each sortedAndFilteredItems as item (item.id)}
			<div class="rounded-lg border bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
				...
				<div class="mt-4 flex justify-between">
					<button
						on:click={() => moveToList(item, 'regular-review')}
