...
13. Maintain and monitor test coverage:

- Use pytest-cov to generate coverage reports.
- Aim to maintain or improve coverage levels with each new feature or change.
- Review coverage reports regularly to identify areas needing additional tests.
...
6. **Continuous Integration**:
   ...
   - Runs all tests: `pytest -v --cov=stojanovic_one --cov-report=xml`
   - Uploads coverage report to Codecov
   - Fails if any tests fail or if coverage drops significantly

7. **Local Testing**:

To run tests locally:

1. Run all non-GUI tests:

   ```
   pytest -v -m "not gui"
   ```

2. Run all tests including GUI tests (may require additional setup on some systems):
...
3. Run tests with coverage:
   ...
   ```
   pytest -v --cov=stojanovic_one
   ```

4. Run non-GUI tests with coverage:
   ```
   pytest -v -m "not gui" --cov=stojanovic_one
   ```

Note: GUI tests may fail on systems without a proper display server. If you encounter issues with GUI tests, focus on the non-GUI tests for development and leave GUI testing to the CI pipeline.

8. **Documentation**: Update relevant documentation as the project evolves.
...
1. **Apply Elon Musk's Algorithm for Efficiency**:
   ...
   c. Simplify and optimize: Only after unnecessary dumb elements are removed optimize and simplify your problem.
...
5. **Error Handling and Security**:
   ...
   - Implement rate limiting for sensitive operations like login attempts.
   - Regularly review and update security measures, including token expiration and rate limiting parameters.
